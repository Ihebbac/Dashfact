...
const ClientInvoicesModal = ({ visible, onCancel, invoices }) => {
  ...
  const customerStats = invoices.reduce((acc, invoice) => {
    ...
    acc[invoice.customerId].totalInvoices += 1;
    acc[invoice.customerId].totalAmount += invoice.total;
    acc[invoice.customerId].totalPayed += invoice.payed;
    acc[invoice.customerId].totalNotPayed += invoice.notpayed;
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <div>
          ...
          <br />
          <Text type="secondary">{record.customerAddress}</Text>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Tag color={
          record.totalNotPayed === 0 ? "green" : 
          record.totalPayed > 0 ? "orange" : "red"
        }>
          {record.totalNotPayed === 0 ? "Tout payé" : 
           record.totalPayed > 0 ? "Partiel" : "Non payé"}
        </Tag>
      ),
      align: "center",
    },
    ...
    const invoiceColumns = [
      ...
      },
      {
        ...
        render: (status) => (
          <Tag color={
            status === "paid" ? "green" :
            status === "partially_paid" ? "orange" : "red"
          }>
            {status === "paid" ? "Payée" :
             status === "partially_paid" ? "Partielle" : "Non payée"}
          </Tag>
        ),
      },
    ];
    ...
    >
      <Space direction="vertical" style={{ width: "100%" }}>
        ...
        <Table
          ...
          pagination={false}
          summary={(pageData) => {
            const totalAmount = pageData.reduce((sum, item) => sum + item.totalAmount, 0);
            const totalPayed = pageData.reduce((sum, item) => sum + item.totalPayed, 0);
            const totalNotPayed = pageData.reduce((sum, item) => sum + item.totalNotPayed, 0);
